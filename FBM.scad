...
module FB(d,w,h)
{
    ...
    difference()
    {
        union()
        {
            scale([1,0.9988,1]) crimp_edge(); //scale because the cut below half messes with the width where the edge meets the ball
            //edge_rounded();
            difference(){ball(); cut();}
        }
        union()
...
//        bez_edge = [[peak,0],[peak,offset*2],[0,d-offset*3],[0,d]];
        bez_edge = [[0,0],[peak/4*pf,0],[peak*pf,0],[peak*pf,offset*0.2],[peak*pf,offset],[peak/4*pf,offset],[0,offset]];
//        !union() {debug_bezier(bez_edge, N=len(bez_edge)-1); #square([18,25]);}
        ...
        //path = concat(path1,[[0,offset],[0,d]]);
